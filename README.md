...
| `--actions` | | _(all)_ | Comma-separated sweep steps to run, in fixed order: `classify`, `changelog`, `approve`, `merge`, `refresh`, `retry`, `remedy`, `mark` (see [Actions](#actions)) |
...
`changelog` is the entry a bot PR earns, in the repositories that still cut their release from the changelog file. A repository with a `cliff.toml` generates its release notes from its commits, so the update is published on the release page already and no entry is written; marge reads the repository and refuses it with that reason. The sweep's `changelog` step writes the entry once the checks are green and **before** it approves anything, and that PR then waits for the CI the commit started; the next sweep classifies the new head and merges it. `enabled` is the switch and is true. A team that does not want its bot PRs to carry an entry sets `enabled: false` in its own file, and a repository that keeps no changelog file costs the sweep one read and nothing else. A PR that already carries the line is left alone, so a team does not stall on the same PR every day, and a repository with no such file is decided exactly as it would have been: a changelog is a courtesy, never a guard. The `changelog` tool writes on demand whatever `enabled` says. `path` is the file, `heading` and `section` say where in it the entry goes, and `template` is the line, a Go text template over `Dependency`, `From`, `To`, `PR`, `Repository`, `Kind` and `UpdateType`. `From` is empty when the title names only the version the update moves to. A template that does not parse, or that names a value the list does not carry, is an error: a team reads back the line it wrote. The section is created under the heading when the file does not carry it, and the heading under the file's title when it carries neither.

`concurrency` has a ceiling marge owns: `perTeam` reaches 20, `perRepo` reaches 5, and the two multiply to at most 20 PRs in flight. GitHub answers a burst of writes with a secondary rate limit, so a policy that declares more is an error rather than a sweep that fails halfway. The product is checked on the resolved policy, so `perTeam: 20` in a team file is an error when the company file already raised `perRepo`. `bot-prs-sweep/policy.schema.json` checks each key against its own range; JSON Schema cannot multiply, so the pair is marge's to refuse.

A refusal for rate reaches marge as a 403, the same status GitHub answers for a protection the caller may not read. The two are told apart by what the answer carries: a rate refusal names the wait, in `Retry-After` or in the reset time beside an exhausted quota. Every client of the process shares one transport, which waits the time GitHub named and sends the request again, up to three times. While it waits, every other call of the process waits with it: a refusal is about the token, not about the request, so letting the rest keep sending is what earned it. A wait longer than a minute, and a refusal that outlives the retries, reach the caller and are reported. The read path is where this matters: a refresh of four teams classifies 60 PRs per team on one caller token.
...
#### Repository settings that hold a queue

A guard that refuses a merge for a repository reason refuses it on every sweep, and the PRs count as `waiting` run after run. The sweep therefore reports the setting itself, under `findings` in the JSON output and as one line per cause in the team's summary. It changes nothing else: the same PRs are refused for the same reasons, and the fix belongs to the repository.
...
A finding is recorded on the PR the setting holds, and the summary groups the findings by cause with a count of the PRs and the repositories: a team sweeps up to 78 repositories, and one line per repository does not reach a reader. The JSON output carries every repository, with the count of each and the required context that never reported.

#### Labels and evidence

Every PR the sweep touched carries exactly one `marge/<class>` label, replaced on each sweep: `merged`, `auto-merge`, `eligible`, `pending`, `action-required`, `held`, `awaiting-approval`, `security`, `stale`, `conflict`, `ci-unavailable`, `skipped`. Labels are display only; no guard reads them back. A label marge may not write is a note on the entry, never a different outcome. A `bot-prs-sweep/<class>` label from the earlier namespace is removed on the next sweep that touches the PR.

An action marge performed, or a guard decision a person needs to see, is written once as an evidence comment: `update-branch`, `retry`, `merged-past-red-check`, `awaiting-approval`, `security-blocked`, `held`. The `held` comment names the update type that waits and the policy files that decided it, which the label does not carry. Evidence is an [ai-rescue marker](#rescue-markers-prior-ai-rescue-attempts) with `"kind":"evidence"` and `"tool":"marge"`, so it carries the head SHA and the diff fingerprint. A second sweep on the same change writes nothing: the fingerprint, not the SHA, decides, so a Renovate rebase does not repeat the comment.

#### Actions

`--actions` runs a subset of the sweep steps, always in this order: `classify` (read the PR, its checks and markers, decide the state, write the label; always runs), `changelog` (commit the team's changelog entry on a PR whose checks are green, before anything is approved, unless the team's policy switches it off or the repository generates its release notes from its commits; the PR then waits for the CI that commit started, and the next sweep merges it), `approve`, `merge`, `refresh` (update stale branches from their base), `retry` (re-run auto-cancelled CircleCI builds on the same head), `remedy` (apply the catalogue rule that matches the classification; see [Rules](#rules)), `mark` (write markers and evidence comments). `remedy` needs `mark` and is refused without it: the one-attempt-per-change guard reads the evidence marker, so a remedy that writes none repeats on every sweep. `--dry-run` decides every outcome and writes nothing, labels included.
...
`match.check.name` reads the checks that failed. `match.check.state: pending` reads the checks that have not finished instead, and it requires `match.check.output.pattern`, an expression over the title, summary and text the check itself reports. A gate check is why: it stays pending until the job it waits for reports, so it is neither a failing check nor a context nobody reported, and its message is the only thing it says about the head. The pattern needs a capture group named `command`, every match is kept, and an action reads the captures rather than composing a string of its own. A pending signal takes no `match.log` and no `baseHead`: a check that never ran has no log, and the rule selects no failing check for a base state to answer for.
...
`marge rules validate` decodes and validates every document; `marge rules test` replays the recorded scenarios under `rules/testdata` against the catalogue. Both run in CI as ordinary Go tests, so a rule without one scenario that matches it and one that refuses it cannot merge. [docs/patterns.md](docs/patterns.md) carries the failure patterns the engine does not act on and the hazards behind the guards. [skills/marge](skills/marge/SKILL.md) is the skill a person loads in Claude Code to drive a sweep; [its hazards reference](skills/marge/references/hazards.md) condenses those hazards and the known-pattern hints for the model that runs when no rule matched. The repository is a Claude Code plugin (`.claude-plugin/plugin.json`), which the Giant Swarm marketplace pins by commit. The AgentTemplate that loads the skill as a commit-pinned git source is not in this repository yet.
...
Starts an MCP server exposing five tools: `list`, `sweep`, `remedy`, `mark` and `changelog`. Each one is an adapter over the same engine call the CLI makes, with the same guards, so a tool call and the equivalent `marge sweep` invocation do the same thing. Every tool that can change a PR takes `dry_run` and carries the MCP read-only and destructive annotations, so a client knows what a call costs before making it.
...
- **`list`** -- read-only. The open bot PRs of a scope with the classification of each. The result has the same shape as `sweep`'s, so "what is waiting for us" and "what would a sweep do" are one question. `refresh` chooses where the classification comes from, and the two readings cost very different amounts:
  - `refresh: false` (the default) reads back the classification the last sweep stored in the PR's `marge/<class>` label, which the PR search already carries. The whole call costs the search, whatever the number of PRs. Several states share one label, so the `status` is the class's representative -- `pending` covers a wait, a cancelled build and a retry, `stale` covers a refresh -- and each of those states asks the reader for the same next step; the evidence, `update_type`, `policy` and `rescue` each need the PR itself and are left out. A PR no sweep has labelled is reported under `unclassified` rather than guessed at.
  ...
  `teams` covers several teams in one call, each read under its own team file and policy. The answer is then `{"teams": [{"team": ..., "result": ...}]}`, one entry per team in the order given, and a team whose files are missing or unreadable carries an `error` instead of a `result`. The teams share one discovery, so reading every team costs one listing rather than one per team. It is refused together with `team`, `query`, `org`, `repos` and `repos_file`.
- **`sweep`** -- mirrors `marge sweep`, returning structured JSON (`rules`, `unhandled`, `summary`, `merged`, `security_failures`, `action_required`, `stale`, `refreshed`, `cancelled`, `retried`, `obsolete`, `waiting`, `ci_unavailable`, `ci_no_verdict`, `skipped`, `eligible`, `unclassified`, `repositories_failed`, `findings`). Each `obsolete` entry carries a `reason` of `superseded` or `no_op`. Each `findings` entry carries a `cause`, the count of the PRs it holds and the repositories it holds them in. `team` selects the team scope; `query`, `org`, `repos` (a list of `org/repo` entries) and `repos_file` (a file in the `--repos-file` format) belong to the query scope and are refused together with `team`. `actions` selects the sweep steps like `--actions`, `changelog` among them, and `prs` narrows the run to the listed PRs (each a PR URL or `owner/repo#number`) like `--prs`; the scope still decides which repositories are read and under which policy, so a PR outside it is refused rather than swept. `teams` sweeps several teams in one call, each under its own team file and policy, and answers in the same `{"teams": [...]}` shape `list` does; the sweeps run one team after another, so two teams that own the same repository never write to it at the same time, and with `prs` each named PR is swept under the team that owns its repository. Each PR entry includes `policy` (the resolved policy the PR was decided under, with the `sources` that produced it), `kind`, `update_type`, `label` (the label on the PR after the sweep; absent when nothing was written, as in `dry_run`), `created_at`, `age_days`, `dependency`, `version_from` and `version_to` (the title read as an update: what moves, and between which versions; a title that names no source version leaves `version_from` empty, and a title in no known shape leaves all three out), and -- when a prior rescue attempt was found -- a `rescue` object (`tool`, `outcome`, `reason`, `at`, `stale`, `rebased`). `rebased: true` means the PR head moved since the attempt but the diff did not (a Renovate rebase); such a marker is still valid and `stale` is `false`. Agent orchestrators should dispatch on `action_required` only, skip entries whose rescue is not `stale` (rebased or not), and escalate those to a human.
...
- **`changelog`** -- writes one changelog entry on each PR named in `prs`, in the format the `team`'s policy declares, and commits it to the PR's own branch. It is the sweep's `changelog` step asked for by name, on the PRs a person picked, whatever the team's policy says about the sweep. `dry_run` returns the line each PR would receive and writes nothing. Each entry of the answer carries the `line`, the `path`, whether it was `written` and the `commit` it made, or the reason it was `refused`: a PR whose author is not a trusted bot, whose branch lives in a fork, that is not open, whose repository has no such file or generates its release notes from its commits, or that already carries the line. Asking twice writes once.
