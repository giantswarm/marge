...
`changelog` is the entry a bot PR earns. The sweep's `changelog` step writes it on a PR the sweep would merge, **before** it approves anything, and that PR then waits for the CI the commit started; the next sweep classifies the new head and merges it. `enabled` is the switch and is true: a dependency update nobody records is a release note nobody can write. A team that does not want its bot PRs to carry an entry sets `enabled: false` in its own file, and a repository that keeps no changelog file costs the sweep one read and nothing else. A PR that already carries the line is left alone, so a team does not stall on the same PR every day, and a repository with no such file is decided exactly as it would have been: a changelog is a courtesy, never a guard. The `changelog` tool writes on demand whatever `enabled` says. `path` is the file, `heading` and `section` say where in it the entry goes, and `template` is the line, a Go text template over `Dependency`, `From`, `To`, `PR`, `Repository`, `Kind` and `UpdateType`. `From` is empty when the title names only the version the update moves to. A template that does not parse, or that names a value the list does not carry, is an error: a team reads back the line it wrote. The section is created under the heading when the file does not carry it, and the heading under the file's title when it carries neither.
...
- **`changelog`** -- writes one changelog entry on each PR named in `prs`, in the format the `team`'s policy declares, and commits it to the PR's own branch. It is the one write no sweep and no rule reaches: a changelog entry is a person's decision about their own repository. `dry_run` returns the line each PR would receive and writes nothing. Each entry of the answer carries the `line`, the `path`, whether it was `written` and the `commit` it made, or the reason it was `refused`: a PR whose author is not a trusted bot, whose branch lives in a fork, that is not open, whose repository has no such file, or that already carries the line. Asking twice writes once.
