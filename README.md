...
Documents are decoded strictly, so an unknown key is an error. A rule whose only signal is a check name is refused: a name says which job went red, never why, so such a rule must carry a log signal, a PR signal, or the `log-matched` refusal. **A rule cannot do what its action forbids**: each action enforces its own guards, a rule may only add refusals, and there is no syntax for removing one. The guard vocabulary is the trusted bot author, no failing security check, the required checks, checks settled on the head, one attempt per change, a log excerpt behind every write, and the generated files a remedy may never hand-edit. Each action carries the subset it needs, not all of them, and `marge rules validate` prints the set an action enforces. Rules are tried in name order and the first match wins, so a catalogue decides the same way however it was read.
