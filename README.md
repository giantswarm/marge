...
Documents are decoded strictly, so an unknown key is an error. **A rule cannot do what its action forbids**: each action enforces its own guards, a rule may only add refusals, and there is no syntax for removing one. The guards are the trusted bot author, no failing security check, the required checks, one attempt per change, a log excerpt behind every write, and the generated files a remedy may never hand-edit. Rules are tried in name order and the first match wins, so a catalogue decides the same way however it was read.
