...
def test_api_working(kube_cluster: Cluster) -> None:
    """The cluster under test answers the Kubernetes API."""
...
def deployment(request, kube_cluster: Cluster) -> List[pykube.Deployment]:
    logger.info("Waiting for the marge deployment..")

    deployment_ready = wait_for_deployment(kube_cluster)

    logger.info("marge deployment looks satisfied..")
...
def test_pods_available(kube_cluster: Cluster, deployment: List[pykube.Deployment]):
    """marge serve came up and its readiness probe passed."""
    for s in deployment:
