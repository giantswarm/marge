...
// TestNewAppClientUsesAPITransport: the scheduled sweep authenticates as the
// App, and its requests go through the App transport, so the pool has to sit
// under that one too.
func TestNewAppClientUsesAPITransport(t *testing.T) {
	client, err := NewAppClient(&App{}, defaultBaseURL, nil)
	require.NoError(t, err)

	transport, ok := client.Client().Transport.(*appTransport)
	require.True(t, ok)
	require.Same(t, APITransport(), transport.base)
}
